...
const Shipments = () => {
  ...
  const fetchData = async () => {
    try {
      const [shipRes, invRes, userRes] = await Promise.all([
        api.get('/shipments'),
        api.get('/inventory'),
        api.get('/users/me')
      ]);
  ...
  const getStatusBadge = (status) => {
    switch (status?.toLowerCase()) {
      case 'delivered':
        return <Badge type="success">{t('dashboard.status_delivered') || 'Delivered'}</Badge>;
      case 'in_transit':
      case 'in transit':
        return <Badge type="info">{t('dashboard.status_in_transit') || 'In Transit'}</Badge>;
      case 'pending':
        return <Badge type="warning">{t('dashboard.status_pending') || 'Pending'}</Badge>;
      default:
        return <Badge type="default">{status || t('dashboard.status_unknown') || 'Unknown'}</Badge>;
    }
  };
  ...
  const updateStatus = async (id, status) => {
    try {
      ...
      showNotification(`${t('shipments.status_updated_to') || 'Shipment marked as'} ${status.replace('_', ' ')}`);
      fetchData();
    } catch (err) {
      console.error('Failed to update status', err);
      showNotification(t('shipments.status_error') || 'Failed to update shipment status', 'error');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showNotification(t('shipments.create_success') || 'Shipment created successfully');
    } catch (err) {
      console.error(err);
      showNotification(t('shipments.create_error') || 'Failed to create shipment', 'error');
    } finally {
    ...
    <div className="space-y-6 fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">{t('shipments.subtitle') || 'Monitor logistics and carbon footprint per delivery.'}</p>
        </div>
        <button 
      ...
      <div className="glass-card rounded-2xl p-6">
        <h3 className="text-lg font-semibold text-white mb-6">{t('shipments.active_shipments') || 'Active Shipments'}</h3>
        ...
        {loading ? (
          ...
          <EmptyState 
            ...
            description={t('shipments.empty_desc') || 'Create your first shipment to start tracking.'}
            icon={Navigation}
            action={
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-slate-800/80 text-slate-300 uppercase text-xs tracking-wider border-b border-slate-700/50">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium">{t('shipments.route') || 'Route'}</th>
                  <th className="px-6 py-4 font-medium">{t('shipments.vehicle') || 'Vehicle'}</th>
                      ...
                      <div className="flex flex-col">
                        ...
                        {shipment.recommendedVehicle && shipment.recommendedVehicle !== shipment.vehicleType && (
                          <div className="text-[10px] text-slate-400 mt-0.5">
                            Recommended: <span className="text-emerald-400 capitalize">{shipment.recommendedVehicle}</span>
                          </div>
                        )}
                        {shipment.savingsKg > 0 && (
                          <div className="text-[10px] text-emerald-500 font-medium mt-0.5">
                            Est. Savings: {shipment.savingsKg?.toFixed(1)} kg
                          </div>
                        )}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="glass-card rounded-2xl w-full max-w-md p-6 relative border border-white/10 shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <Truck className="w-5 h-5 text-primary" /> {t('shipments.create_title') || 'Create New Shipment'}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-300">{t('shipments.select_item') || 'Select Inventory Item'}</label>
                <select
                  required
                ...
                >
                  <option value="" disabled>{t('shipments.select_product_placeholder') || 'Select a product...'}</option>
                  {inventory.map(item => (
                    <option key={item._id} value={item._id}>{item.productName} ({item.quantity} {t('inventory.in_stock') || 'in stock'})</option>
                  ))}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-300">{t('shipments.origin_city') || 'Origin City'}</label>
                  <input
                    type="text" required
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-300">{t('shipments.dest_city') || 'Destination City'}</label>
                  <input
                    type="text" required
                    className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-2 px-3 text-white focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-300">{t('shipments.vehicle_type') || 'Vehicle Type'}</label>
                  <select
                    required
                    className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-2 px-3 text-white focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 appearance-none"
                    value={formData.vehicleType} onChange={(e) => setFormData({...formData, vehicleType: e.target.value})}
                  >
                    <option value="truck">{t('settings.vehicle_truck') || 'Truck'}</option>
                    <option value="van">{t('settings.vehicle_van') || 'Van'}</option>
                    <option value="rail">{t('settings.vehicle_rail') || 'Rail'}</option>
                    <option value="ship">{t('settings.vehicle_ship') || 'Ship'}</option>
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-300">{t('shipments.distance') || 'Distance (km)'}</label>
                  <input
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" disabled={formLoading} className="bg-primary hover:bg-emerald-400 text-dark px-4 py-2 rounded-xl font-medium transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)] disabled:opacity-50">
                  {formLoading ? t('shipments.creating') || 'Creating...' : t('shipments.create_button') || 'Create Shipment'}
                </button>
              </div>
            </form>
          </div>
