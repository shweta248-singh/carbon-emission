...
import { Activity, Leaf, TrendingDown, Wind } from 'lucide-react';
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [summaryRes, vehicleRes, trendsRes] = await Promise.all([
        ...
        api.get('/analytics/emission-trend')
      ]);
  ...
  const renderTrendChart = () => {
    if (trends.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-slate-500 py-20">
          <Activity className="w-12 h-12 mb-3 opacity-20" />
          <p className="text-center font-medium">No trend data available yet.</p>
        </div>
      );
    }
    ...
    return (
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={trends} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          />
          <Legend />
          <Area type="monotone" dataKey="carbonEmissions" stroke="#ef4444" fillOpacity={1} fill="url(#colorEmission)" name={t('dashboard.carbon_emissions')} />
          <Area type="monotone" dataKey="co2Saved" stroke="#10b981" fillOpacity={1} fill="url(#colorSaved)" name={t('dashboard.co2_saved')} />
        </AreaChart>
      </ResponsiveContainer>
    );
  };
